...
namespace {
    class generatorParametersTestFixture: public testing::TestWithParam<std::tuple<generatorParameters, solvedMasks>> {
    };

    INSTANTIATE_TEST_SUITE_P (
        generatorParametersTests,
        generatorParametersTestFixture,
        ::testing::ValuesIn(testDataGenerators::combine2VectorsLinear<generatorParameters, solvedMasks> (
            {
                {   // LL
                    { 4, 5, 6 },                // lockedEdges
                    { },                        // ignoredEdges
    ...
    TEST_P(generatorParametersTestFixture, toSolvedMasksTest) {
        const auto &[params, expected] = GetParam();
        ASSERT_EQ(expected.edgesMask, params.toSolvedMasks().edgesMask);
        ASSERT_EQ(expected.cornersMask, params.toSolvedMasks().cornersMask);
    }
}
